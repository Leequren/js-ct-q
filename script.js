let allowNumbers = '0123456789\t', allowLetters = 'йцукнгзхъфывапролджэячсмитьбюqwertyuiopasdfghjklzxcvbnm '
...
function checkLetter(letter, containerLetters) {
    return containerLetters.includes(letter)
}
...
card.addEventListener('input', (event) => {
    ...
    if (!checkLetter(card.value[card.value.length - 1], allowNumbers)) {
        card.value = card.value.slice(0, card.value.length - 1)
    }
...
person.addEventListener('input', (event) => {
    event.preventDefault()
    if (checkLetter(person.value[person.value.length - 1], allowLetters)) {
        personCur.textContent = person.value
    } else {
        person.value = person.value.slice(0, person.value.length - 1)
    }
...
codeForm.addEventListener('input', (event) => {
    event.preventDefault()
    console.log(codeForm.value[codeForm.value.length - 1], checkLetter(codeForm.value[codeForm.value.length - 1], '0123456789'))
    if (checkLetter(codeForm.value[codeForm.value.length - 1], '0123456789')) {
        codeCur.textContent = codeForm.value
    } else {
        codeForm.value = person.value.slice(0, codeForm.value.length - 1)
    }
})
...
validityForm.addEventListener('input', (event) => {
    ...
    if (!checkLetter(validityForm.value[validityForm.value.length - 1], '0123456789/')) {
        validityForm.value = validityForm.value.slice(0, validityForm.value.length - 1)
    }
...
btn.addEventListener('click', (event) => {
    ...
    if (newCardInfo[0] && formElements.card.value.length === 19 && newCardInfo[2].length === 3 && newCardInfo[3].length === 5 && +newCardInfo[3].slice(0, 2) <= 12) {
        ...
        console.log(newCardInfo[3].length === 5, +newCardInfo[3].slice(0, 2))
        if (newCardInfo[3].length === 5 && (+newCardInfo[3].slice(0, 2) > 12))
            alert('Проверьте дату')
        else
            alert('Ошибки в заполнении формы карты')
    }
})